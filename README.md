...
这是一个java单元测试的工具类，核心两个功能：
json校验器：这是一个功能强大的 JSON 校验工具库，提供了灵活的 JSON 比较和自定义校验功能。
           在java的单元测试中，可以轻松地使用这个工具库进行 JSON 数据的校验，并在测试失败时，会显示详细的差异信息，帮助开发人员快速定位问题。

restful API接口参数边界值测试用例自动生成：这是一个基于SpringBoot的RESTful API接口参数边界值自动化测试用例自动生成工具。

目前已完成了json的校验器功能，接口边界值自动化生成功能未来实现。
...
如果你有任何问题或者新的功能需求，欢迎提issue，或加我的QQ讨论：549958058。
如果你愿意的话，请打赏我10块钱，如果你有大厂的工作介绍，请联系我
![img_1.png](img_1.png)